...
d3.json("https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_week.geojson").then(function (data) {
  ...
  function getColor(depth) {
    ...
    if (depth < 10) return '#f1c40f'
    else if (depth < 20) return '#f39c12'
    else if (depth < 30) return '#e67e22';
    else if (depth < 40) return '#d35400';
    else if (depth < 50) return '#c0392b';
    else return '#a93226';
  };
  ...
  legend.onAdd = function () {
    ...
    const depthColors = ['#f1c40f', '#f39c12', '#e67e22', '#d35400', '#c0392b', '#a93226']
    ...
    for (let i = 0; i < depthIntervals.length; i++) {
      div.innerHTML +=
        '<li style="background:' + depthColors[i] + '"></li> ' + depthIntervals[i] + (depthIntervals[i + 1] ? '&ndash;' + depthIntervals[i + 1] + '<br>' : '+');
    }
